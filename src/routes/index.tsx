...
function HomePage() {
  return (
    <div>
      ...
      {/* SECTION DIVIDER */}
      <section className="bg-white py-14 sm:py-20 border-t border-border">
        <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-10 grid gap-8 sm:gap-12 md:grid-cols-12 items-end">
          <div className="md:col-span-7">
            <span className="text-[10px] uppercase tracking-[0.35em] text-gold">The Houses</span>
            <h2 className="mt-4 sm:mt-5 font-serif text-4xl sm:text-5xl md:text-7xl leading-[0.95] text-navy">
              A family of brands,
      ...
      {/* FEATURED COMPANIES */}
      <section className="bg-white pb-20 sm:pb-32 px-5 sm:px-6 lg:px-10">
        <div className="mx-auto max-w-7xl grid gap-8 sm:gap-10 sm:grid-cols-2 lg:grid-cols-3">
          {companies.map((c) => (
            <a
              key={c.name}
              href={c.url}
            ...
            >
              <div className="relative overflow-hidden rounded-2xl bg-white shadow-[0_30px_80px_-30px_rgba(20,30,80,0.25)] aspect-[4/3]">
                <img
                  ...
                  alt={c.name}
                  loading="lazy"
                  className="h-full w-full object-contain p-6 sm:p-8 transition-transform duration-[1.4s] ease-out group-hover:scale-[1.06]"
                />
                ...
                <div className="absolute top-4 left-4 sm:top-6 sm:left-6 text-[9px] sm:text-[10px] tracking-[0.2em] sm:tracking-[0.3em] text-white bg-navy/70 backdrop-blur-sm px-2.5 sm:px-3 py-1.5 rounded-full">
                  {c.n} — {c.tag}
                </div>
                <div className="absolute top-4 right-4 sm:top-6 sm:right-6 h-9 w-9 sm:h-11 sm:w-11 rounded-full bg-white/90 backdrop-blur flex items-center justify-center text-navy group-hover:bg-gold transition-all">
                  <ArrowUpRight className="h-4 w-4 sm:h-5 sm:w-5 transition-transform group-hover:rotate-45" />
                </div>
              </div>
              <div className="mt-5 sm:mt-6 flex items-start justify-between gap-6">
                <div>
                  <h3 className="font-serif text-2xl sm:text-3xl md:text-4xl text-navy group-hover:text-gold transition-colors">
                    {c.name}
                  </h3>
      ...
      <section className="relative bg-navy text-white py-20 sm:py-32 px-5 sm:px-6 lg:px-10 overflow-hidden">
        <div
          aria-hidden
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <div className="mt-14 sm:mt-20 grid gap-x-8 sm:gap-x-12 gap-y-10 sm:gap-y-14 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
            {[
              ...
              <div key={v.t} className="border-t border-white/15 pt-5 sm:pt-6">
                <div className="text-gold text-xs tracking-[0.3em]">0{i + 1}</div>
                <div className="mt-3 font-serif text-2xl sm:text-3xl text-white">{v.t}</div>
                <p className="mt-3 text-sm text-white/65 leading-relaxed">{v.d}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* CTA */}
      <section className="bg-white py-20 sm:py-32 px-5 sm:px-6 lg:px-10">
        <div className="mx-auto max-w-4xl text-center">
          <span className="text-[10px] uppercase tracking-[0.35em] text-gold">Let's talk</span>
          <h2 className="mt-4 sm:mt-5 font-serif text-4xl sm:text-5xl md:text-7xl text-navy leading-[1.05] sm:leading-[1]">
            Partner with a group that <span className="italic gradient-gold-text">builds to last.</span>
          </h2>
          <Link
            to="/contact"
            className="mt-8 sm:mt-10 inline-flex items-center gap-2 bg-navy text-white px-7 sm:px-8 py-3.5 sm:py-4 rounded-full text-sm tracking-wide hover:bg-navy-deep transition-all group"
          >
            Get in touch
            <ArrowUpRight className="h-4 w-4 transition-transform group-hover:rotate-45" />
          </Link>
